...
#AutoIt3Wrapper_Res_Fileversion=5.3.5.2
...
Dim $PROG_ID = "D5902C4BE60D4234E361443265F260D2B8A28A73", $PNP_DIAG, $TRAY[27][20], $FLOPPY, $PNP[27][2]
...
Dim $MENU[100], $GUI_ONLY = False, $DIR = "", $SDIR = "", $ARun, $z1, $SVCPACK = 0, $IDLE = 90, $DELAY = 0
...
Dim $DBEG = _Iif(@Compiled = 0, "(Uncompiled)", "v" & FileGetVersion(@ScriptFullPath)) & "\" & _
		StringReplace(@OSVersion, "WIN_", "") & " " & StringReplace(@OSServicePack, "Service Pack ", "SP") & "\" & @ProcessorArch
Dim $TITLE = "Drive Space Indicator"
Dim $PROC = _Iif(@ProcessorArch = "X64", "64", "")
...
If $CMDLINE[0] > 0 Then
	For $i = 1 To $CMDLINE[0]
		...
		ElseIf $CMDLINE[$i] = "/PROGTRAY"  Then
			$PROGTRAY[0] = 1
		ElseIf $CMDLINE[$i] = "/PROGTRAY:0"  Then
			$PROGTRAY[0] = 0
		ElseIf $CMDLINE[$i] = "/LIBICON"  Then
			$LIBICON = 1
		ElseIf $CMDLINE[$i] = "/LIBICON:0"  Then
			$LIBICON = 0
		ElseIf StringLeft($CMDLINE[$i], 7) = "/DELAY:"  Then
			$DELAY = _Max(_Min(9, StringReplace($CMDLINE[$i], "/DELAY:", "")), 1)
		ElseIf StringLeft($CMDLINE[$i], 6) = "/IDLE:"  Then
			$IDLE = _Max(_Min(100, StringReplace($CMDLINE[$i], "/IDLE:", "")), 0)
...
While 1
	...
	For $i = 0 To UBound($Chk) - 1
		...
		For $j = 1 To $List[0]
			...
			If $i = 0 Then ; Decide whether optical drive is CD, DVD or virtual.
				...
				If @error Then $ICON = _Iif(StringInStr($Type, "DVD") <> 0, 2, _Iif(StringInStr($Type, "CD") <> 0, 1, 3))
				RegWrite($PNPKEY, $Type, "REG_DWORD", $ICON)
				If DriveStatus($Drv) = "READY"  Then $ICON = _Iif($ICON = 3, 6, _Iif(DriveSpaceTotal($Drv) <= 700, 4, 5))
	...
	If $BMON = 1 Then _GUI_Tray_Populate()
	_GUI_Drive_Icon()
	...
	If $CONFIG = 1 Then
		...
		While $GUI <> 0
			Sleep(1000)
		WEnd
		If MsgBox(0x41024, $TITLE, _FormatStr("Main_06", $TITLE)) = 7 Then Exit
		$CONFIG = 0
	EndIf
...
;==================================================================================
; Functions handling beta status elevation:
;==================================================================================
Func _BetaElevate()
	If $BETA = 1 Then $BETA = 2
EndFunc   ;==>_BetaElevate
...
Func _PNP_Scan()
	...
	;====================================================================================
	Local $UsbDev, $Filt1 = "", $Filt2 = "", $stComputer = "localhost"
	Local $Drv = DriveGetDrive("REMOVABLE")
	If @error = 0 Then
		For $y = 1 To $Drv[0]
			$UsbDev = RegRead($MOUNT, "\DosDevices\" & $Drv[$y])
			$UsbDev = _HexToString(StringReplace($UsbDev, "00", ""))
			If $UsbDev <> "" And StringInStr($UsbDev, "STORAGE#RemovableMedia", 2) = 0 Then $FLOPPY = $FLOPPY & "|" & $Drv[$y]
		Next
		If $FLOPPY <> "" Then
			;$Filt1 = """' And Dependent <> '\\\\" & $stComputer & "\\root\\cimv2:Win32_LogicalDisk.DeviceID="""
	...
	$objWMIService = ObjGet("winmgmts:\\" & $stComputer & "\root\CIMV2")
...
Func _PNP_Get($TDrv)
	Local $a
	For $a = 1 To $PNP[0][0]
		If $TDrv = $PNP[$a][0] Then Return $PNP[$a][1]
	Next
	Return _HexToString(StringReplace(RegRead($MOUNT, "\DosDevices\" & $TDrv), "00", ""))
EndFunc   ;==>_PNP_Get
...
Func _DisplayError($MSG, $ERR = 1)
	MsgBox(0x41010, $TITLE, _Iif($ERR = 1, _ReadStr("ERROR") & ": ", "") & $MSG)
	Exit
...
Func _LaunchWeb($Link)
	Local $TEMP = RegRead("HKCU" & $PROC & "\Software\Clients\StartMenuInternet", "")
	If $TEMP = "" Then $TEMP = "IEXPLORE.EXE"
	Run(RegRead("HKLM" & $PROC & "\SOFTWARE\Clients\StartMenuInternet\" & $TEMP & "\shell\open\command", "") & " """ & $Link & """")
	If @error = 1 Then MsgBox(0x30, $TITLE, _ReadStr("Main_10"))
EndFunc   ;==>_LaunchWeb
		...
		If StringRight($TList[$a], 4) = ".cab"  Then
			...
			If FileExists(@TempDir & "\DSI\*.dll") = 0 Then
				If FileExists(@TempDir & "\DSI\*.icl") = 0 Then
					MsgBox(0, $TITLE, _FormatStr("Menu_1H3", $TList[0]))
				Else
...
Func Menu_StartAtBoot()
	...
	If $RUN Then
		RegWrite($RUNKEY, "DriveSpace", "REG_SZ", @ScriptFullPath & _Iif($BETA, " /BETA", "") & _Iif($DELAY, " /DELAY:" & $DELAY, ""))
	Else
...
Func Menu_Redetect()
	If MsgBox(0x41024, $TITLE, _ReadStr("Main_08")) = 6 Then
		RegDelete($PNPKEY)
		$BMON = 1
		$Timer[0] = 0
	EndIf
...
Func Menu_LangPM()
	...
	If $Link = "" Then
		MsgBox(0x10, $TITLE, _ReadStr("Menu_3C1"))
	Else
		_LaunchWeb($Link)
	EndIf
EndFunc   ;==>Menu_LangPM
...
; <--- About menu --->
Func Menu_RyanVM()
	_LaunchWeb("http://www.ryanvm.net/forum/viewtopic.php?t=5454")
	_Reduce_Memory()
EndFunc   ;==>Menu_RyanVM

Func Menu_WinCert()
	_LaunchWeb("http://www.wincert.net/forum/index.php?showtopic=4781")
	_Reduce_Memory()
EndFunc   ;==>Menu_WinCert

Func Menu_OnTheWeb()
	_LaunchWeb("http://xptsp.filetap.com/DSI_Home.htm")
	_Reduce_Memory()
...
Func Menu_About()
	...
	GUICtrlCreateLabel(_FormatStr("Menu_4A1", "2007-2008") & " Douglas Orend", 60, 30, 280, 20)
...
Func GUI_Clipboard()
	...
	MsgBox(0, $TITLE, _ReadStr("GUI_4B1"))
	_Reduce_Memory()
...
Func GUI_Manage_Delete()
	Local $ID = $SC[0][1]
	If MsgBox(0x24, $TITLE, _ReadStr("GUI_3D1") & @CRLF & $SC[$ID][1]) = 6 Then
...
Func _GUI_Drive_Icon($Drive = $DRV_SEL, $ICL = "", $GRP = -1)
	...
	GUICtrlSetImage($BOX[10], $ICL, _Iif($PROC = "64", -1, 1) * $ICN + 1)
	$DRV_SEL = $Drive
...
Func Updates_Build($Auto = 1)
	...
	If (@IPAddress1 = "0.0.0.0"  Or @IPAddress1 = "127.0.0.1") And @IPAddress2 = "0.0.0.0"  _
		...
		If $Auto = False Then MsgBox(0x10, $TITLE, _ReadStr("Upd_01"))
		Return
	EndIf
	...
	If InetGet($WEBSITE & "Version.txt", $File, 1) = 0 Then
		If $Auto = 0 Then MsgBox(0x40, $TITLE, _ReadStr("Upd_02"))
		_Reduce_Memory()
		Return
	EndIf
	...
	If $Auto Then
		If $b = 1 Then Return
		If MsgBox(0x24, $TITLE, StringReplace(_ReadStr(_Iif($ISBETA, "Upd_12", "Upd_08")), "|", @CRLF)) = 7 Then
			_GUI2_Close()
			Return
		EndIf
	Else
		If $b = 1 Then
			MsgBox(0x40, $TITLE, _ReadStr("Upd_02"))
			_GUI2_Close()
			Return
		EndIf
	EndIf
		...
		If $LV[$a][4] = 1 Then
			...
			If InetGet($WEBSITE & $LV[$a][6], @TempDir & "\" & $File, 1, 1) Then
				...
				If StringRight($File, 4) <> ".exe"  Then
					...
					FileDelete(@TempDir & "\" & $File)
				Else
					Local $S = RegRead($RUNKEY, "DriveSpace"), $i = StringInStr($S, "/DELAY:")
					$S = _Iif($i <> 0, StringMid($S, $i, 8), "") & " " & _Iif($BETA, " /BETA", "")
					RegDelete($KEY, "Next_Update")
					Run(@TempDir & "\" & $File & " -gm2 /SILENT " & $S)
					Leave_Script()
				EndIf
			EndIf
		EndIf
...
Func Install_Cancel()
	...
	If MsgBox(0x24, $TITLE, StringReplace(_FormatStr("Inst_1E", $TITLE), "|", @CRLF)) = 6 Then Exit
...
Func Install_Page4()
	...
	If $GUI2 <> 0 Then
		...
		If FileExists($DIR) And $CONFIG = 2 Then
			If MsgBox(0x24, _ReadStr("Inst_4B"), StringReplace(_FormatStr("Inst_4C", $DIR), "|", @CRLF)) = 7 Then Return
		EndIf
	EndIf
...
Func Install_Exit()
	Local $S = RegRead($RUNKEY, "DriveSpace"), $i = StringInStr($S, "/DELAY:")
	$S = _Iif($i <> 0, StringMid($S, $i, 8), "") & " " & _Iif($BETA, " /BETA", "")
	If $RUN = 1 Then RegWrite($RUNKEY, "DriveSpace", "REG_SZ", $DIR & "\DrvSpace.exe " & $S)
...
Func _Program_Remove()
	...
	If MsgBox(0x24, $TITLE, _FormatStr("REM_01", $TITLE)) = 7 Then Exit
	...
	DirRemove($SDIR)
	ProgressOff()
	MsgBox(0x40, $TITLE, StringFormat($DONE, $TITLE))
