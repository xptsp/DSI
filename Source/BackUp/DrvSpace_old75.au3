...
#AutoIt3Wrapper_Res_Fileversion=5.3.6.1
...
Func _PNP_Scan()
	...
	;====================================================================================
	Local $i = 1, $colItems, $j, $Antecedent, $Dependent, $DL
	Const $wbemFlagReturnImmediately = 0x10, $wbemFlagForwardOnly = 0x20

	; Let's try to filter out the floppy drives first:
	$DL = DriveGetDrive("REMOVABLE")
	If @error = 0 Then
		For $j = 1 To $DL[0]
			$usbdev = RegRead("HKLM\SYSTEM\MountedDevices", "\DosDevices\" & StringLeft($DL[$j], 1))
			$usbdev = _HexToString(StringReplace($usbdev, "00", ""))
			If StringInStr($usbdev, "STORAGE#RemovableMedia") Then $FLOPPY = $FLOPPY & $DL[$j]
		Next
	EndIf
	...
	For $objItem In $colItems
		...
		For $j = 1 To $i - 1
			If $DeviceID = $PNP[$j][1] Then $PNP[$j][1] = $objItem.PNPDeviceID
		Next
	Next
...
Func GUI_Select_Drive()
	Local $a
	For $a = 1 To $TRAY[0][0]
		If $TRAY[$a][2] = @GUI_CtrlId Then
			If StringInStr($FLOPPY, $TRAY[$a][0]) Then
				GUICtrlSetState($TRAY[$a][2], $GUI_UNCHECKED)
			Else
				_GUI_Drive_Icon($TRAY[$a][0], "", $TRAY[$a][1])
			EndIf
		endif
;### Tidy Error -> "next" is closing previous "if" on line 1586
	Next
;### Tidy Error -> "endfunc" is closing previous "for" on line 1585
...
;### Tidy Error -> "func" cannot be inside any IF/Do/While/For/Case/Func statement.
	Func _Program_Remove()
		...
		Exit
	EndFunc   ;==>_Program_Remove;### Tidy Error -> func is never closed in your script.
