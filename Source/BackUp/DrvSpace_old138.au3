		...
		For $j = 1 To $List[0]
			...
			Else
				...
				ElseIf $i = 3 Then ; Figure out what kind of removable drive this is:
					...
					EndIf
					;MsgBox(0,$DRV & "(" & $G & ")", "$TYPE = " & $TYPE & @CRLF & "$FLOPPY = " $FLOPPY)
					$PNP_DIAG = $PNP_DIAG & StringUpper($Drv) & " = " & $G & "," & $Type & @CR
					RegWrite($PNPKEY, $Type, "REG_DWORD", $G)
					$ARun = ""
				EndIf
...
Func _PNP_Scan()
	...
	;====================================================================================
	Local $UsbDev, $Drv = DriveGetDrive("REMOVABLE")
	If @error = 0 Then
		For $y = 1 To $Drv[0]
			$UsbDev = RegRead($MOUNT, "\DosDevices\" & $Drv[$y])
			$UsbDev = _HexToString(StringReplace($UsbDev, "00", ""))
			If $UsbDev <> "" And StringInStr($UsbDev, "FDC#GENERIC_FLOPPY_DRIVE", 2) = 0 Then $FLOPPY = $FLOPPY & "|" & $Drv[$y]
		Next
	EndIf
