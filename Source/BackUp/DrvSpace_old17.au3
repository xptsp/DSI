...
#AutoIt3Wrapper_Res_Fileversion=5.4.0.0
...
Dim $DBEG = _Iif(@Compiled = 0, "(Uncompiled)", "v" & FileGetVersion(@ScriptFullPath)) & "\" & _
		StringReplace(@OSVersion, "WIN_", "") & " " & StringReplace(@OSServicePack, "Service Pack ", "SP") & "\" & @CPUArch
Dim $TITLE = "Drive Space Indicator"
Dim $PROC = _Iif(@CPUArch = "X64", "64", "")
...
$colDiskDrives = $objWMIService.ExecQuery("SELECT * FROM Win32_DiskDrive", "WQL", _
		$wbemFlagReturnImmediately + $wbemFlagForwardOnly)

For $objDrive In $colDiskDrives
	$strDeviceID = StringReplace($objDrive.DeviceID, "\", "\\")
	$PNP = $objDrive.PNPDeviceID

	$colPartitions = $objWMIService.ExecQuery("ASSOCIATORS OF {Win32_DiskDrive.DeviceID=""" & $strDeviceID & _
			"""} WHERE AssocClass = Win32_DiskDriveToDiskPartition")
	For $objPartition In $colPartitions
		$colLogicalDisks = $objWMIService.ExecQuery("ASSOCIATORS OF {Win32_DiskPartition.DeviceID=""" & _
				$objPartition.DeviceID & """} WHERE AssocClass = Win32_LogicalDiskToPartition")

		For $objLogicalDisk In $colLogicalDisks
			$PNP[$x][0] = $objLogicalDisk.DeviceID
			$PNP[$x][1] = $PNP
			$x += 1
		Next
	Next
