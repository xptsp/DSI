...
#AutoIt3Wrapper_Res_Fileversion=5.3.6.0
...
While 1
	...
	For $i = 0 To UBound($Chk) - 1
		...
		For $j = 1 To $List[0]
			...
			$TRAY[$z][0] = $Drv
			$TRAY[$z][1] = $G
			RegWrite($ICN & StringUpper(StringLeft($Drv, 1)) & "\DefaultIcon", "", "REG_SZ", $THEME_FILE & "," & $ICON + 1)
			if @OSArch = "X64" then
				RegWrite($ICN64 & StringUpper(StringLeft($Drv, 1)) & "\DefaultIcon", "", "REG_SZ", $THEME_FILE & "," & $ICON + 1)
			endif
...
Func _PNP_Scan()
	...
	;====================================================================================
	Local $objWMIService = ObjGet("winmgmts:\\" & @ComputerName & "\root\CIMV2"), $x = 0
	Local $colDiskDrives = $objWMIService.ExecQuery("SELECT * FROM Win32_DiskDrive")

	For $objDrive In $colDiskDrives
		Local $strDeviceID = StringReplace($objDrive.DeviceID, "\", "\\")
		Local $DeviceID = $objDrive.PNPDeviceID
