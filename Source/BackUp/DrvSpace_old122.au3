...
Func _PNP_Scan()
	...
	If @error = 0 Then
		For $j = 1 To $DL[0]
			...
			If StringInStr($usbdev, "FDC#GENERIC_FLOPPY_DRIVE") = 0 Then $FLOPPY = $FLOPPY & $DL[$j]
		Next
	EndIf
...
Func _GUI_Drive_Icon($Drive = $DRV_SEL, $ICL = "", $GRP = -1)
	...
	MsgBox(0 ,"",_SetIcon($BOX[10], $ICL, $ICN, 48, 48))
	;GUICtrlSetImage($BOX[10], $ICL, -1 * (_Iif($PROC = "64", -1, 1) * $ICN))
	$DRV_SEL = $Drive
